...
extern "C" void stats_event(const char* e);

VL_INLINE_OPT void Vmips_core___024unit____Vdpiimwrap_stats_event_TOP____024unit(std::string e) {
    VL_DEBUG_IF(VL_DBG_MSGF("+        Vmips_core___024unit____Vdpiimwrap_stats_event_TOP____024unit\n"); );
    // Body
    const char* e__Vcvt;
    for (size_t e__Vidx = 0; e__Vidx < 1; ++e__Vidx) e__Vcvt = e.c_str();
    stats_event(e__Vcvt);
}
